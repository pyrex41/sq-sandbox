...
pub const S3Client = struct {
    ...
    // ── Sync Modules (bidirectional) ─────────────────────────────────

    /// Bidirectional sync of modules between local disk and S3.
    /// Push local-only .squashfs to modules/, pull remote-only to local.
    /// Matches sync_modules in bin/sq-s3 and Rust s3/sync.rs.
    pub fn syncModules(
        self: *const S3Client,
        allocator: std.mem.Allocator,
        modules_dir: []const u8,
    ) void {
        const local_names = listLocalSquashfs(allocator, modules_dir) catch return;
        defer {
            for (local_names) |n| allocator.free(n);
            allocator.free(local_names);
        }

        var local_set = std.StringHashMap(void).init(allocator);
        defer local_set.deinit();
        for (local_names) |n| {
            local_set.put(n, {}) catch return;
        }

        // Push local-only modules
        for (local_names) |name| {
            var key_buf: [512]u8 = undefined;
            const key = std.fmt.bufPrint(&key_buf, "modules/{s}.squashfs", .{name}) catch continue;
            var path_buf: [std.fs.max_path_bytes]u8 = undefined;
            const local_path = std.fmt.bufPrint(&path_buf, "{s}/{s}.squashfs", .{ modules_dir, name }) catch continue;

            if (self.exists(allocator, key)) |exists_val| {
                if (exists_val) continue;
            } else |_| continue;

            self.push(allocator, local_path, key) catch |err| {
                log.warn("sync_modules: push failed for {s}: {}", .{ name, err });
                continue;
            };
            log.info("sync_modules: push (local-only): {s}", .{name});
        }

        // Pull remote-only modules
        const remote_keys = self.list(allocator, "modules/") catch |err| {
            log.warn("sync_modules: list failed: {}", .{err});
            return;
        };
        defer freeList(allocator, remote_keys);

        for (remote_keys) |key| {
            // key is e.g. "modules/000-base-alpine.squashfs" or "000-base-alpine.squashfs"
            const name = blk: {
                if (std.mem.startsWith(u8, key, "modules/")) {
                    break :blk key["modules/".len..];
                }
                break :blk key;
            };
            ...
            if (local_set.contains(stem)) continue;

            var dest_buf: [std.fs.max_path_bytes]u8 = undefined;
            const dest = std.fmt.bufPrint(&dest_buf, "{s}/{s}", .{ modules_dir, name }) catch continue;

            var pull_key_buf: [512]u8 = undefined;
            const pull_key = if (std.mem.startsWith(u8, key, "modules/")) key else std.fmt.bufPrint(&pull_key_buf, "modules/{s}", .{key}) catch continue;

            self.pull(allocator, pull_key, dest) catch |err| {
                log.warn("sync_modules: pull failed for {s}: {}", .{ name, err });
                continue;
            };
            log.info("sync_modules: pull (remote-only): {s}", .{name});
        }
    }
...
/// List .squashfs files in a directory. Returns module names (stem without .squashfs).
/// Caller owns the returned slice and each string; free each then the slice.
fn listLocalSquashfs(allocator: std.mem.Allocator, modules_dir: []const u8) ![][]const u8 {
    ...
    while (iter.next() catch null) |entry| {
        ...
        const stem = entry.name[0 .. entry.name.len - ".squashfs".len];
        if (stem.len == 0) continue;
        try list.append(allocator, try allocator.dupe(u8, stem));
    }
