...
/// 3. If S3 enabled: sync modules (push local-only, pull remote-only)
/// 4. Ensure 000-base-alpine.squashfs exists (native S3 pull or sq-mkbase)
/// 5. If firecracker backend: check/build VM components
/// 6. If not ephemeral: remount surviving sandboxes (with S3 pull for missing modules)
pub fn run(cfg: *const config.Config) void {
    ...
    // S3 sync at startup (matches Rust init flow)
    if (cfg.s3Enabled()) {
        if (s3_mod.S3Client.fromEnv()) |s3_client| {
            var modules_buf: [256]u8 = undefined;
            if (cfg.modulesDir(&modules_buf)) |modules_dir| {
                log.info("syncing modules from S3", .{});
                s3_client.syncModules(allocator, modules_dir);
            } else |_| {}
        }
    }

    ensureBaseImage(cfg, allocator);
    ...
    if (!cfg.ephemeralEnabled()) {
        remountSurvivingSandboxes(cfg, allocator);
    }
...
fn ensureBaseImage(cfg: *const config.Config, allocator: std.mem.Allocator) void {
    ...
    std.fs.accessAbsolute(base_path, .{}) catch {
        ...
        if (cfg.s3Enabled()) {
            if (s3_mod.S3Client.fromEnv()) |s3_client| {
                if (s3_client.pull(allocator, "modules/000-base-alpine.squashfs", base_path)) |_| {
                    log.info("base image pulled from S3", .{});
                    return;
                } else |err| {
                    log.warn("S3 pull failed for base: {}", .{err});
                }
            }
        }
...
/// Pulls missing modules from S3 when configured.
fn remountSurvivingSandboxes(cfg: *const config.Config, allocator: std.mem.Allocator) void {
    var sandboxes_buf: [256]u8 = undefined;
    ...
    var s3_storage: ?s3_mod.S3Client = null;
    if (cfg.s3Enabled()) {
        if (s3_mod.S3Client.fromEnv()) |client| {
            s3_storage = client;
        }
    }
    const s3_client_ptr: ?*const s3_mod.S3Client = if (s3_storage) |*s| s else null;

    var count: usize = 0;
    var iter = dir.iterate();
    while (iter.next() catch null) |entry| {
        if (entry.kind != .directory) continue;
        remountOneSandbox(cfg, sandboxes_dir, entry.name, allocator, s3_client_ptr);
        count += 1;
    }
...
fn remountOneSandbox(
    ...
    s3_client: ?*const s3_mod.S3Client,
) void {
    ...
    while (lines.next()) |line| {
        ...
        std.fs.accessAbsolute(mod_path, .{}) catch {
            if (s3_client) |s3| {
                ...
                s3.pull(allocator, s3_key, mod_path) catch {
                    log.warn("sandbox {s}: module {s} not found (local and S3)", .{ id, trimmed });
                    continue;
                };
                log.info("sandbox {s}: pulled missing module {s} from S3", .{ id, trimmed });
            } else {
                log.warn("sandbox {s}: module {s} not found at {s}", .{ id, trimmed, mod_path });
                continue;
            }
