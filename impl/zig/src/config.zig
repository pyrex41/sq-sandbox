...
pub const Config = struct {
    ...
    pub fn secretsPath(self: *const Config, buf: []u8) ![]const u8 {
        return std.fmt.bufPrint(buf, "{s}/secrets.json", .{self.data_dir});
    }

    /// Path to VM directory: {data_dir}/vm
    pub fn vmDir(self: *const Config, buf: []u8) ![]const u8 {
        return std.fmt.bufPrint(buf, "{s}/vm", .{self.data_dir});
    }

    /// Check if secrets.json exists (proxy should start when true).
    ...
    pub fn fromEnv() Config {
        ...
        // Prefer SQUASH_DATA_DIR, fallback to legacy SQUASH_DATA
        if (getEnvNonEmpty("SQUASH_DATA_DIR")) |v| {
            cfg.data_dir = v;
        } else if (getEnvNonEmpty("SQUASH_DATA")) |v| {
            cfg.data_dir = v;
        }
...
test "secretsPath builds correct path" {
    const cfg = Config{ .data_dir = "/mnt/data" };
    var buf: [256]u8 = undefined;
    const path = try cfg.secretsPath(&buf);
    try std.testing.expectEqualStrings("/mnt/data/secrets.json", path);
}

test "vmDir builds correct path" {
    const cfg = Config{ .data_dir = "/mnt/data" };
    var buf: [256]u8 = undefined;
