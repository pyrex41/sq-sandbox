...
Conn_Channel :: struct {
	...
	not_empty: sync.Cond,
	not_full:  sync.Cond,
	buf:       [CONN_QUEUE_CAP]Conn_Work_Item, // fixed-size ring buffer — zero heap alloc
...
conn_channel_init :: proc(ch: ^Conn_Channel) {
	ch.head = 0
	ch.tail = 0
	ch.count = 0
	ch.closed = false
...
conn_channel_send :: proc(ch: ^Conn_Channel, item: Conn_Work_Item) -> bool {
	sync.mutex_lock(&ch.mu)
	for ch.count >= CONN_QUEUE_CAP && !ch.closed {
	...
	ch.buf[ch.tail] = item
	ch.tail = (ch.tail + 1) % CONN_QUEUE_CAP
	ch.count += 1
...
conn_channel_recv :: proc(ch: ^Conn_Channel) -> (item: Conn_Work_Item, ok: bool) {
	...
	item = ch.buf[ch.head]
	ch.head = (ch.head + 1) % CONN_QUEUE_CAP
	ch.count -= 1
