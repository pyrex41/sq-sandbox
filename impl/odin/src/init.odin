...
// Run an external command via direct fork/execvp. Returns true on success (exit 0).
// Avoids system() which double-forks through /bin/sh.
run_cmd :: proc(args: ..string) -> bool {
	if len(args) == 0 {
		return false
	}

	// Build null-terminated argv array on temp allocator
...
// Run an external command and capture its stdout. Returns (stdout, success).
// Uses a stack buffer — output is truncated at 64KB.
run_cmd_capture :: proc(args: ..string) -> (output: string, ok: bool) {
	if len(args) == 0 {
		return "", false
	}
	...
	total: int = 0
	for total < len(buf) {
		n := c_read(stdout_fds[0], raw_data(buf[total:]), c.size_t(len(buf) - total))
		if n <= 0 {
			break
		}
		total += int(n)
	...
	return string(buf[:total]), success
}

// Check if a command exists by searching PATH directories.
