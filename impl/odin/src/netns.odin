...
netns_setup :: proc(
	...
	if nets, ok := allow_net.?; ok && len(nets) > 0 {
		cn := fmt.aprintf("squash-%s", id, allocator = allocator)
		_apply_egress_rules(cn, veth_host, nets)
		chain_name = cn
	}
...
// Creates a custom chain and links it to FORWARD for the given interface.
_apply_egress_rules :: proc(
	...
	for host in hosts {
		...
		if !_is_literal_allow_target(host) {
			// Avoid shell interpolation for hostnames; require proxy mode for domain policies.
			fmt.printfln("[net] WARN: hostname %s requires proxy mode or CIDR/IP allow_net", host)
			continue
		}
		run_cmd("iptables", "-A", chain, "-d", host, "-j", "ACCEPT")
	}
...
// True for literal IP/CIDR targets only (IPv4/IPv6/CIDR-ish syntax).
_is_literal_allow_target :: proc(host: string) -> bool {
	if len(host) == 0 {
		return false
	}
	for ch in host {
