...
(defun firecracker-setup-network (id index allow-net)
  "Set up tap device and NAT for a firecracker VM.
   ...
   ALLOW-NET is optional network policy input from API payload.
  ...
  (let ((tap (format nil "sq-~A-tap" id))
    ...
    ;; Optional egress allow-list policy via dedicated FORWARD chain.
    (let* ((raw-hosts (cond
                        ((null allow-net) nil)
                        ((stringp allow-net) (list allow-net))
                        ((listp allow-net) allow-net)
                        (t nil)))
           (hosts (remove-if (lambda (h)
                               (or (not (stringp h))
                                   (string= h "none")
                                   (search "*" h)))
                             raw-hosts)))
      (dolist (h raw-hosts)
        (when (and (stringp h) (search "*" h))
          (warn "firecracker allow-net wildcard requires proxy mode: ~A" h)))
      (when hosts
        (apply-egress-rules id tap hosts)))
    tap))
...
(defun firecracker-teardown-network (id index)
  ...
  (let ((tap (format nil "sq-~A-tap" id))
        (subnet (format nil "10.0.~D.0/30" index))
        (chain (format nil "SQ-~A" id)))
    ;; Remove optional egress chain (best effort).
    (ignore-errors
      (run-command "iptables" "-D" "FORWARD" "-i" tap "-j" chain))
    (ignore-errors
      (run-command "iptables" "-F" chain))
    (ignore-errors
      (run-command "iptables" "-X" chain))
    ;; Remove NAT rule
    (ignore-errors
