...
// Firecracker-specific handle stored alongside sandbox state.
Firecracker_Handle :: struct {
	cid:       u32,
	tap_index: int,
}
...
firecracker_setup_network :: proc(id: string, index: int, allow_net: Maybe([]string)) -> Firecracker_Error {
	...
	if nets, ok := allow_net.?; ok && len(nets) > 0 {
		...
		run_cmd("iptables", "-A", "FORWARD", "-i", tap_name, "-j", chain)
		...
		for host in nets {
			...
			if strings.contains(host, "*") {
				fmt.printfln("[fc-net] WARN: wildcard %s requires proxy mode", host)
				continue
			}
			run_cmd("sh", "-c",
...
firecracker_start_vm :: proc(
	...
	// Build command: sq-firecracker start {id} --cpu {cpu} --mem {mem} --cid {cid} {paths...}
	buf: [4096]byte
	b := strings.builder_from_bytes(buf[:])
	fmt.sbprintf(&b, "sq-firecracker start %s --cpu %.1f --mem %d --cid %d", id, cpu, memory_mb, cid)
...
firecracker_exec :: proc(
	...
	// Build JSON request — escape cmd string for safe embedding
	req_buf: [65536]byte
	...
	// Build shell command to pipe JSON through socat
	// echo '<json>' | socat -T{timeout} - VSOCK-CONNECT:{cid}:5000
	shell_buf: [65536 + 256]byte
	shell_b := strings.builder_from_bytes(shell_buf[:])
	// Use printf to avoid echo interpretation of escape sequences
	strings.write_string(&shell_b, "printf '%s' '")
	// Escape single quotes in the JSON for shell embedding
	for ch in json_req {
		if ch == '\'' {
			strings.write_string(&shell_b, "'\"'\"'")
		} else {
			strings.write_rune(&shell_b, ch)
		}
	}
	fmt.sbprintf(&shell_b, "' | socat -T%d - VSOCK-CONNECT:%d:5000", timeout_s, cid)

	shell_cmd := strings.to_string(shell_b)

	// Execute via a temporary file to capture output
	// Write script, run it, capture stdout
	// Sandbox lock is held during exec, so CID alone is unique enough
	tmp_out := fmt.tprintf("/tmp/sq-fc-exec-%d.out", cid)
	full_cmd := fmt.tprintf("%s > %s 2>/dev/null", shell_cmd, tmp_out)
	cstr := strings.clone_to_cstring(full_cmd, context.temp_allocator)
	rc := c_system(cstr)

	finished := time.now()

	// Read response
	resp_data, read_ok := os.read_entire_file(tmp_out, context.temp_allocator)
	os.remove(tmp_out)

	if !read_ok || len(resp_data) == 0 {
		if rc != 0 {
			return Exec_Result{
				exit_code = 124,
				stdout    = "",
	...
	// Parse JSON response
	resp: Vsock_Response
	if json.unmarshal(resp_data, &resp, allocator = allocator) != nil {
